...
class ViralIntelligence:
    ...
    def build_cards(self, posts: list[dict], limit: int = 8) -> list[dict]:
        ...
        for item in posts:
            ...
            if not text:
                continue
            topic_label = self._topic(str(item.get("query", "")), text, str(item.get("topic", "")).strip())
            score = self._score(item)
            if "geopolitics" in topic_label or "politics" in topic_label:
                required_keywords = ["market", "rates", "trade", "btc", "crypto", "oil", "liquidity", "sanctions", "fed", "treasury", "etf", "macro"]
                if not any(k in text.lower() for k in required_keywords):
                    score *= 0.1
